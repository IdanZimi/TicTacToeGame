...
let logicalBoard = Array(9).fill(null)
...
const cellClicked = (event) => {
    ...
    if (!logicalBoard[id - 1]) {
        logicalBoard[id - 1] = currPlayer;
        ...
        currPlayer = currPlayer === player_X ? player_O : player_X; // switch player
...
function checkIfThereIsAWinner(){
    ...
    winningCombinations.forEach(combination => {
        if (logicalBoard[combination[0] - 1] == currPlayer
            && logicalBoard[combination[1] - 1] == currPlayer
             && logicalBoard[combination[2] - 1] == currPlayer) {
